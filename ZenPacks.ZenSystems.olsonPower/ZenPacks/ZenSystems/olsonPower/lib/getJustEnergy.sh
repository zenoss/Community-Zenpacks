...
energy_days=$(/usr/bin/snmpget  -r 10 -t 5 -$3 -c $2  -O qvU  $1 1.3.6.1.4.1.17933.1.1.22 2>/dev/null)
energy_days=$(echo $energy_days | bc -l 2>/dev/null)

if [ -n "$energy_days" ]
then
  echo "OK|energy_days=$energy_days"
else echo "Unknown"
fi
