...
from ZenPacks.community.CiscoASAVPN.zenasavpn import Device, Process
from Products.ZenHub.PBDaemon import translateError

def getCiscoASAVPNConf(dev):
    """
    ...
    if not dev.snmpMonitorDevice():
        return None
    procs = dev.getMonitoredComponents(collector='zenasavpn')
    if not procs:
        return None
    d = Device()
    d.name = dev.id
...
class CiscoASAVPNConfig(PerformanceConfig):
    ...
    def getCiscoASAVPNConf(self, devices = None):
        ...
        for dev in self.config.devices():
            ...
            dev = dev.primaryAq()
            try:
                procinfo = getCiscoASAVPNConf(dev)
                if procinfo:
    ...
    @translateError
    def remote_getCiscoASAVPNConf(self, devices=None):
        return self.getCiscoASAVPNConf(devices)


    @translateError
    def remote_getCiscoASAVPNStatus(self, devices=None):
