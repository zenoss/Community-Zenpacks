...
Gets SNMP process data from a device's Cisco Remote Access Service MIB
...
log = logging.getLogger("zen.zenasavpn")
...
HOSTROOT  ='.1.3.6.1.4.1.9.9.392.1.3.21.1'
NAMETABLE = HOSTROOT + '.2'
InPktsT    = HOSTROOT + '.31.'        # note trailing dot
OutPktsT   = HOSTROOT + '.32.'        # note trailing dot
InDropT    = HOSTROOT + '.33.'
OutDropT   = HOSTROOT + '.34.'
InOctetsT  = HOSTROOT + '.35.'
OutOctetsT = HOSTROOT + '.36.'

DEFAULT_PARALLEL_JOBS = 10

# Max size for CPU numbers
WRAP = 0xffffffffffffffffL
...
class Pid:
    ...
    def updateInPkts(self, n):
        """
        """
        self.inpkts = n

    def updateOutPkts(self, n):
        """
        """
        self.outpkts = n

    def updateInDrops(self, n):
        """
        """
        self.indrops = n

    def updateOutDrops(self, n):
        """
        """
        self.outdrops = n

    def updateInOctets(self, n):
        """
        """
        self.inoct = n

    def updateOutOctets(self, n):
        """
        """
        self.outoct = n
    ...
    def __str__(self):
        ...
        """
        return '<Pid> In Packets: %s Out Packets: %s' % (self.inpkts,self.outpkts)
        __repr__ = __str__
...
class Process(pb.Copyable, pb.RemoteCopy):
    ...
    def match(self, name):
        ...
        @rtype: Boolean
        """
        #log.debug('Match request for %s with original name of %s'%(name,self.originalName))
    ...
    def updateInPkts(self, pid, value):
        """
        """
        p = self.pids.setdefault(pid, Pid())
        inpkts = p.updateInPkts(value)
	log.debug('Value received for inpkts: %s-%s'%(value,inpkts))
    ...
    def updateOutPkts(self, pid, value):
        """
        """
        p = self.pids.setdefault(pid, Pid())
        outpkts = p.updateOutPkts(value)
    ...
    def updateInDrops(self, pid, value):
        """
        """
        p = self.pids.setdefault(pid, Pid())
        indrops = p.updateInDrops(value)
        if indrops is not None:
    ...
    def updateOutDrops(self, pid, value):
        """
        """
        p = self.pids.setdefault(pid, Pid())
        outdrops = p.updateOutDrops(value)
        if outdrops is not None:
            self.outdrops += inpkts
            self.outdrops %= WRAP

    def getOutDrops(self):
        """
        """
        return self.outdrops

    def updateInOctets(self, pid, value):
        """
        """
        p = self.pids.setdefault(pid, Pid())
        inoct = p.updateInOctets(value)
    ...
    def updateOutOctets(self, pid, value):
        """
        """
        p = self.pids.setdefault(pid, Pid())
        outoct = p.updateOutOctets(value)
...
class Device(pb.Copyable, pb.RemoteCopy):
    ...
    def get(self, oids):
        ...
        """
        res =  self.proxy.get(oids,
                              self.snmpConnInfo.zSnmpTimeout,
                              self.snmpConnInfo.zSnmpTries)
	log.debug("Get results returns: %s"%str(res))
	return res
...
class zenasavpn(SnmpDaemon):
    ...
    statusEvent = { 'eventClass' : Status_OSProcess,
                    'eventGroup' : 'Process' }
    initialServices = SnmpDaemon.initialServices + ['ZenPacks.community.CiscoASAVPN.services.CiscoASAVPNConfig']
    ...
    def __init__(self, noopts=False):
        SnmpDaemon.__init__(self, 'zenasavpn', noopts)
        self._devices = {}
        self.scanning = None
    ...
    def fetchConfig(self):
        ...
        def doFetchConfig(driver):
            ...
            yield self.model().callRemote('getCiscoASAVPNConf', devices)
            driver.next()
            self.sendEvents(
    ...
    def remote_updateDeviceList(self, devices):
        ...
        if updated:
            ...
            d = self.model().callRemote('getCiscoASAVPNConf', devices)
    ...
    def start(self, driver):
        ...
        yield self.model().callRemote('getCiscoASAVPNStatus', self.options.device)
	...
	log.debug('Done showProcessList')
		...
		log.debug('Checking process %s with %s is %s'%(name,pid,p.match(name[0])))
                if p.match(name[0]):
                    log.debug("Found process %s on %s" % (pid, p.name))
                    after[pid] = p
        ...
        for p in new:
            log.debug("Found new %s pid %s on %s" % (
    ...
    def storePerfStats(self, results, device):
        ...
        @type device: Device object
        """
	log.debug('In storePerfStats')
        for success, result in results:
            #log.debug('Got Result %s : %s' %(success,result))
            if not success:
	...
	#log.debug('Got Parts: %s'%parts)
        ...
        for pidConf, pids in byConf.items():
            ...
            for pid in pids:
                log.debug('Got Here')
                inpkts = results.get(InPktsT + str(pid), None)
		log.debug('Inpkts: %s'%inpkts)
                outpkts = results.get(OutPktsT + str(pid), None)
                indrops = results.get(InDropT + str(pid), None)
                outdrops = results.get(OutDropT + str(pid), None)
                inoctets = results.get(InOctetsT + str(pid), None)
                outoctets = results.get(OutOctetsT + str(pid), None)
                pidConf.updateInPkts(pid, inpkts)
                pidConf.updateOutPkts(pid, outpkts)
                pidConf.updateInDrops(pid, indrops)
                pidConf.updateOutDrops(pid, outdrops)
                pidConf.updateInOctets(pid, inoctets)
                pidConf.updateOutOctets(pid, outoctets)
            self.save(device.name, pidName, 'inpkts_inpkts', pidConf.getInPkts(),
                      'DERIVE', min=0)
            self.save(device.name, pidName, 'outpkts_outpkts', pidConf.getOutPkts(),
                      'DERIVE', min=0)
            self.save(device.name, pidName, 'indrops_indrops', pidConf.getInDrops(),
                      'DERIVE', min=0)
            self.save(device.name, pidName, 'outdrops_outdrops', pidConf.getOutDrops(),
                      'DERIVE', min=0)
            self.save(device.name, pidName, 'inoctets_inoctets', pidConf.getInOctets(),
                      'DERIVE', min=0)
            self.save(device.name, pidName, 'outoctets_outoctets', pidConf.getOutOctets(),
                      'DERIVE', min=0)
...
if __name__ == '__main__':
    ...
    from ZenPacks.community.CiscoASAVPN.zenasavpn import zenasavpn
    z = zenasavpn()
    z.run()
