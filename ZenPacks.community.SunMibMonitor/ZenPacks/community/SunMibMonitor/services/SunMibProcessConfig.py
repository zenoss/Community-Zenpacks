...
from ZenPacks.community.SunMibMonitor.zensunprocess import Device, Process
from Products.ZenHub.PBDaemon import translateError

def getSunMibProcessConf(dev):
    """
    ...
    if not dev.snmpMonitorDevice():
        return None
    procs = dev.getMonitoredComponents(collector='zensunprocess')
    if not procs:
        return None
    d = Device()
    d.name = dev.id
...
class SunMibProcessConfig(PerformanceConfig):
    ...
    def getSunMibProcessConf(self, devices = None):
        ...
        for dev in self.config.devices():
            ...
            dev = dev.primaryAq()
            try:
                procinfo = getSunMibProcessConf(dev)
                if procinfo:
    ...
    @translateError
    def remote_getSunMibProcessConf(self, devices=None):
        return self.getSunMibProcessConf(devices)
